...
public class palindrome {
    ...
    static Pair<Integer, String> theLongestSubPalindrome(String string){
            ...
            for (int i = 1; i < string_length + 1; i++) {
                for (int j = 1; j < string_length + 1; j++) {
                    if (string.charAt(i - 1) == reversed_string.charAt(j - 1))
                        ...
                        substring_length_matrix[i][j] = Math.max(substring_length_matrix[i - 1][j], substring_length_matrix[i][j - 1]);
                }
            }
        ...
        return new Pair<Integer, String>(palindrome_length,the_palindrome.toString());
    }
    ...
    public static void main(String[] args) throws IOException {
        ...
        Pair <Integer, String> answers = theLongestSubPalindrome((in.readLine()).trim());
        if (answers.getKey() != 0)
